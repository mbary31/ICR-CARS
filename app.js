...
const toast=(t)=>{const el=$("#toast"); if(!el) return; el.textContent=t; el.classList.add('show'); setTimeout(()=>el.classList.remove('show'),2000);};
...
function saveAll(arr){ store.setItem(KEY, JSON.stringify(arr)); }
...
// ===== Home =====
async function renderHome(){ const c=$("#ultimo"); if(!c) return; const v=newest(); if(!v){ c.innerHTML='<div class="empty">Sin stock por ahora.</div>'; return;} c.innerHTML=card(v); }
...
function bindAdmin(){
  ...
  fotosIn?.addEventListener('change', async (e)=>{
    ...
    for(const f of Array.from(e.target.files).slice(0,12)){
      const b64 = await toBase64(f); fotos.push(b64);
    }
  ...
  form?.addEventListener('submit', e=>{
    ...
    const a=getAll(); a.unshift(v); saveAll(a);
    toast('Vehículo guardado'); form.reset(); fotos=[]; renderListado(); renderAdminList(); renderHome();
  });
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  renderHome(); renderListado(); bindAdmin(); pintarEjemplo();
