...
function saveAll(arr){
  try { store.setItem(KEY, JSON.stringify(arr)); }
  catch(e){ console.error(e); toast('Límite del navegador. Prueba con menos fotos.'); throw e; }
}
...
function bindAdmin(){
  ...
  fotosIn?.addEventListener('change', async (e)=>{
    ...
    // límites pragmáticos para localStorage (Safari ~5MB para todo el origen)
    const MAX_TOTAL_BYTES = 3_600_000; // ~3.6MB
    let total = 0;
    for(const f of files){
      const b64 = await toBase64Compressed(f, 1200, 0.72); // fuerte compresión
      const bytes = base64Bytes(b64);
      if(total + bytes > MAX_TOTAL_BYTES) { 
        toast('Se limitaron fotos por el tamaño del navegador.');
        break;
      }
      fotos.push(b64);
      total += bytes;
...
function base64Bytes(b64){
  ...
  // aproximación rápida del tamaño real
